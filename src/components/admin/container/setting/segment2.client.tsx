        ...
        <div className="grid grid-cols-3 gap-4 text-xs">
          {layouts.map(({ type }) => (
            <div key={type} className="flex flex-col items-center gap-2">
              <button
                type="button"
                className="w-full rounded-md p-1 cursor-pointer border border-gray-200 dark:border-gray-800"
                style={(setting.layout === type ? { borderColor: setting.color } : {})}
          ...
          {contents.map(({ type }) => (
            <div key={type} className="flex flex-col items-center gap-2">
              <button
                type="button"
                className="w-full rounded-md p-1 cursor-pointer border border-gray-200 dark:border-gray-800"
                style={(setting.content === type ? { borderColor: setting.color } : {})}
